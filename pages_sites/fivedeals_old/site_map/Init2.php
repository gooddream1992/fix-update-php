...
//	printf("<br><font color=blue>Working dir: '%s'</font>\n", str_replace($startdir, '', $path));
	...
	while ($file = readdir($handle)) {
		...
		if(str_replace('.bak', '', $path) != $path) $ignore = true;
		...
		if(str_replace('select/group', '', $path) != $path) $ignore = true;
		if(str_replace('help/cc_required', '', $path) != $path) $ignore = true;
		if(str_replace('accounting/payment_method', '', $path) != $path) $ignore = true;
		if(str_replace('api', '', $path) != $path) $ignore = true;
		if(str_replace('data', '', $path) != $path) $ignore = true;
		if(str_replace('search_zip', '', $path) != $path) $ignore = true;
		if(str_replace('national_internet_deals', '', $path) != $path) $ignore = true;
		if(str_replace('thank_you', '', $path) != $path) $ignore = true;
		if(str_replace('final', '', $path) != $path) $ignore = true;
		if(str_replace('/r/', '', $fullfilename) != $fullfilename) $ignore = true;
...
$siteMapPages = array();
$startdir = '/var/www/vhosts/'.$_SERVER['HTTP_HOST'].'/pages';
