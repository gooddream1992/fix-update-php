...
/**
 * Base class that represents a row from the 'deal_feed_view' table.
 *
 * 
 *
 * @package    propel.generator.komideals.om
 */
abstract class BaseDealFeedView extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'DealFeedViewPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        DealFeedViewPeer
	 */
	...
	/**
	 * Initializes internal state of BaseDealFeedView object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     DealFeedView The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = DealFeedViewPeer::ID;
		}
	...
	public function setBusinessId($v)
	{
		...
		if ($this->business_id !== $v) {
			$this->business_id = $v;
			$this->modifiedColumns[] = DealFeedViewPeer::BUSINESS_ID;
		}
	...
	public function setDateCreated($v)
	{
		...
		if ( $this->date_created !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = DealFeedViewPeer::DATE_CREATED;
			}
	...
	public function setUserAgentId($v)
	{
		...
		if ($this->user_agent_id !== $v) {
			...
			$this->modifiedColumns[] = DealFeedViewPeer::USER_AGENT_ID;
		}
	...
	public function setHttpReferer($v)
	{
		...
		if ($this->http_referer !== $v) {
			$this->http_referer = $v;
			$this->modifiedColumns[] = DealFeedViewPeer::HTTP_REFERER;
		}
	...
	public function setRemoteAddr($v)
	{
		...
		if ($this->remote_addr !== $v) {
			$this->remote_addr = $v;
			$this->modifiedColumns[] = DealFeedViewPeer::REMOTE_ADDR;
		}
	...
	public function setSid($v)
	{
		...
		if ($this->sid !== $v || $this->isNew()) {
			$this->sid = $v;
			$this->modifiedColumns[] = DealFeedViewPeer::SID;
		}
	...
	public function setHttpReferer2($v)
	{
		...
		if ($this->http_referer2 !== $v) {
			...
			$this->modifiedColumns[] = DealFeedViewPeer::HTTP_REFERER2;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->user_agent_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->http_referer = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->remote_addr = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->sid = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->http_referer2 = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->resetModified();
			...
			return $startcol + 8; // 8 = DealFeedViewPeer::NUM_COLUMNS - DealFeedViewPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating DealFeedView object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = DealFeedViewPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				DealFeedViewPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(DealFeedViewPeer::ID) ) {
					...
					$affectedRows += DealFeedViewPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = DealFeedViewPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = DealFeedViewPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = DealFeedViewPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getBusinessId(),
			$keys[2] => $this->getDateCreated(),
			$keys[3] => $this->getUserAgentId(),
			$keys[4] => $this->getHttpReferer(),
			$keys[5] => $this->getRemoteAddr(),
			$keys[6] => $this->getSid(),
			$keys[7] => $this->getHttpReferer2(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setUserAgentId($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setHttpReferer($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setRemoteAddr($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setSid($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setHttpReferer2($arr[$keys[7]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(DealFeedViewPeer::ID)) $criteria->add(DealFeedViewPeer::ID, $this->id);
		if ($this->isColumnModified(DealFeedViewPeer::BUSINESS_ID)) $criteria->add(DealFeedViewPeer::BUSINESS_ID, $this->business_id);
		if ($this->isColumnModified(DealFeedViewPeer::DATE_CREATED)) $criteria->add(DealFeedViewPeer::DATE_CREATED, $this->date_created);
		if ($this->isColumnModified(DealFeedViewPeer::USER_AGENT_ID)) $criteria->add(DealFeedViewPeer::USER_AGENT_ID, $this->user_agent_id);
		if ($this->isColumnModified(DealFeedViewPeer::HTTP_REFERER)) $criteria->add(DealFeedViewPeer::HTTP_REFERER, $this->http_referer);
		if ($this->isColumnModified(DealFeedViewPeer::REMOTE_ADDR)) $criteria->add(DealFeedViewPeer::REMOTE_ADDR, $this->remote_addr);
		if ($this->isColumnModified(DealFeedViewPeer::SID)) $criteria->add(DealFeedViewPeer::SID, $this->sid);
		if ($this->isColumnModified(DealFeedViewPeer::HTTP_REFERER2)) $criteria->add(DealFeedViewPeer::HTTP_REFERER2, $this->http_referer2);
	...
	{
		$criteria = new Criteria(DealFeedViewPeer::DATABASE_NAME);
		$criteria->add(DealFeedViewPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of DealFeedView (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     DealFeedView Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     DealFeedViewPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new DealFeedViewPeer();
		}
		return self::$peer;
	}
	...
	public function getBusiness(PropelPDO $con = null)
	{
		if ($this->aBusiness === null && ($this->business_id !== null)) {
			...
			/* The following can be used additionally to
				 ...
				 $this->aBusiness->addDealFeedViews($this);
			 */
		}
	...
	public function getUserAgent(PropelPDO $con = null)
	{
		if ($this->aUserAgent === null && ($this->user_agent_id !== null)) {
			...
			/* The following can be used additionally to
				 ...
				 undesirable since it could result in an only partially populated collection
				 in the referenced object.
				 $this->aUserAgent->addDealFeedViews($this);
			 */
		}
