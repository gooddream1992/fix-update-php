...
abstract class BaseBusiness extends BaseObject  implements Persistent
{
	...
	/**
	 * @var        array DealFeedView[] Collection to store aggregation of DealFeedView objects.
	 */
	protected $collDealFeedViews;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collDealFeedViews !== null) {
				foreach ($this->collDealFeedViews as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getDealFeedViews() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addDealFeedView($relObj->copy($deepCopy));
				}
			}
	 ...
	 * @return     PropelCollection|array DealFeedClick[] List of DealFeedClick objects
	 */
	public function getDealFeedClicksJoinDealFeed($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = DealFeedClickQuery::create(null, $criteria);
	 ...
	 * @return     PropelCollection|array DealFeedCommission[] List of DealFeedCommission objects
	 */
	public function getDealFeedCommissionsJoinDealFeed($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = DealFeedCommissionQuery::create(null, $criteria);
		$query->joinWith('DealFeed', $join_behavior);
	 ...
	 * @return     PropelCollection|array DealFeedStat[] List of DealFeedStat objects
	 */
	public function getDealFeedStatsJoinDealFeed($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = DealFeedStatQuery::create(null, $criteria);
		$query->joinWith('DealFeed', $join_behavior);
	...
	/**
	 * Clears out the collDealFeedViews collection
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addDealFeedViews()
	 */
	public function clearDealFeedViews()
	{
		$this->collDealFeedViews = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collDealFeedViews collection.
	 *
	 * By default this just sets the collDealFeedViews collection to an empty array (like clearcollDealFeedViews());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initDealFeedViews()
	{
		$this->collDealFeedViews = new PropelObjectCollection();
		$this->collDealFeedViews->setModel('DealFeedView');
	}

	/**
	 * Gets an array of DealFeedView objects which contain a foreign key that references this object.
	 *
	 * If the $criteria is not null, it is used to always fetch the results from the database.
	 * Otherwise the results are fetched from the database the first time, then cached.
	 * Next time the same method is called without $criteria, the cached collection is returned.
	 ...
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 * @return     PropelCollection|array DealFeedView[] List of DealFeedView objects
	 * @throws     PropelException
	 */
	public function getDealFeedViews($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collDealFeedViews || null !== $criteria) {
			if ($this->isNew() && null === $this->collDealFeedViews) {
				// return empty collection
				$this->initDealFeedViews();
			} else {
				$collDealFeedViews = DealFeedViewQuery::create(null, $criteria)
					->filterByBusiness($this)
					->find($con);
				if (null !== $criteria) {
					return $collDealFeedViews;
				}
				$this->collDealFeedViews = $collDealFeedViews;
			}
		}
		return $this->collDealFeedViews;
	}

	/**
	 * Returns the number of related DealFeedView objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related DealFeedView objects.
	 * @throws     PropelException
	 */
	public function countDealFeedViews(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collDealFeedViews || null !== $criteria) {
			if ($this->isNew() && null === $this->collDealFeedViews) {
				return 0;
			} else {
				$query = DealFeedViewQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collDealFeedViews);
		}
	}

	/**
	 * Method called to associate a DealFeedView object to this object
	 * through the DealFeedView foreign key attribute.
	 *
	 * @param      DealFeedView $l DealFeedView
	 * @return     void
	 * @throws     PropelException
	 */
	public function addDealFeedView(DealFeedView $l)
	{
		if ($this->collDealFeedViews === null) {
			$this->initDealFeedViews();
		}
		if (!$this->collDealFeedViews->contains($l)) { // only add it if the **same** object is not already associated
			$this->collDealFeedViews[]= $l;
			$l->setBusiness($this);
		}
	}
	 ...
	 * Otherwise if this Business is new, it will return
	 * an empty collection; or if this Business has previously
	 * been saved, it will retrieve related DealFeedViews from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Business.
	 ...
	 * @return     PropelCollection|array DealFeedView[] List of DealFeedView objects
	 */
	public function getDealFeedViewsJoinUserAgent($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = DealFeedViewQuery::create(null, $criteria);
		$query->joinWith('UserAgent', $join_behavior);

		return $this->getDealFeedViews($query, $con);
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collDealFeedViews) {
				foreach ((array) $this->collDealFeedViews as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
