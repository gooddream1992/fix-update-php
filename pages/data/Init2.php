...
function getEmailMessage($strEmail) {
	...
	if (!$objEmailLead instanceof EmailLead) {
		...
		$objEmailLead->setAffiliateId($dDefaultAffiliateId);
		$objEmailLead->save();
	}
	
	return 'window.parent.submitThisForm();';
}
function saveEmailZip($strEmail, $strPostalCode, $strIp=NULL, $dAid=NULL) {
	global $objSession;
	/* @var $objSession UserSession */
	...
	if (!$objEmailLead instanceof EmailLead) {
		...
		if ($dAid && strlen($dAid)) {
			$objEmailLead->setAffiliateId($dAid);
			$objEmailLead->save();
		}
	}
	return TRUE;
...
if ($_GET['elmid'] == 'blog_data') {
	...
	saveEmailZip($_GET['email_address'], $_GET['postal_code'], $_GET['ip'], $_GET['aid']);
}
