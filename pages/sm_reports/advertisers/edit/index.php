...
<table width="95%" border="0">
	...
	<tr>
		...
		<td class="mainText">
		<a href="/sm_reports-advertisers-accounting.htm?id=<?=$WorkingUser->getId();?>">Accounting/CC Info</a>
		(Last 4: <?=$WorkingUser->getDefaultPaymentMethod()->getNumberLastFour();?>)
		<a href="/sm_reports-advertisers-notes.htm?template_id=9&id=<?=$WorkingUser->getId();?>">email no cc</a>
		</td>
	</tr>
	<tr>
		<td class="mainText"><b>AdGroups</b></td>
		<td class="mainText">
		<?
		
		$AdGroups = $WorkingUser->getAdGroups();
		foreach ($AdGroups as $AdGroup) {
			printf("<a href=\"/sm_reports-ad_groups-edit.htm?show_all=1&id=%d&back_page=%s\">%s</a><br>\n", $AdGroup->getId(), urlencode($_SERVER['REQUEST_URI']), htmlspecialchars($AdGroup->getGroupName()));
		}
...
?>
	...
	<tr>
		...
		<td class="mainText">Login <?=$WorkingUser->getWilshireUsername();?> <input type="button" onclick="submitWilshireForm();" value="Login">
		-- <a href="/sm_reports-advertisers-stats.htm?id=<?=$WorkingUser->getId();?>">Usage Stats</a></td>
	</tr>
<?
...
<form target="_blank" name="wilshireForm" id="wilshireForm" method="POST" action="https://reporting.specificclick.net/Interface/admin/login.asp">
<input type="hidden" name="login" value="<?=htmlspecialchars($WorkingUser->getWilshireUsername());?>">
<input type="hidden" name="password" value="<?=htmlspecialchars($WorkingUser->getWilshirePassword());?>">
</form>
<SCRIPT LANGUAGE="JavaScript">
function submitWilshireForm() {
document.getElementById('wilshireForm').submit();
}
</script>