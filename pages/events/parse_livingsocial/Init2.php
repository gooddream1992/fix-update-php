...
function get_livingsocial_datax($url, $force=FALSE) {
//	return get_deal_data($url);
	return get_livingsocial_data($url, $force);
}
...
if (strlen($_GET['getUrl'])) {
	...
	$arrMatches = check_apache_running_script('events-parse_livingsocial.htm?v_c=via_cron');
	...
	/**
	 * Must get city page to find all cities for looping...
	 * Do not change to deal page unless for temporary testing.
	 */
	
//	$url = 'http://livingsocial.com/deals/12813-5-spin-classes-for-39';
//	$url = 'http://livingsocial.com/deals/9654-one-dozen-cupcakes';
//	$url = 'http://livingsocial.com/deals/45878-4-field-box-tickets-4-hot-dogs-and-4-sodas';
	$url = 'http://livingsocial.com/cities/2-boston';
//	$url = 'http://livingsocial.com/deals/6504-50-off-italian-cuisine';

	$data = get_livingsocial_data($url, true);
//	vdump($data);
//	exit;
	...
	if($data) {
		...
		foreach ($data['deals_on_page'] as $dealUrl) {
			$arrDealUrl = get_livingsocial_data($dealUrl);
			if ($arrDealUrl) {
				save_deal_from_array($arrDealUrl);
			}
		}
		...
		foreach ($data['regions'] as $regionUrl) {
			...
			$url = 'http://'.$_SERVER['HTTP_HOST'].'/events-parse_livingsocial.htm?getUrl='. urlencode($regionUrl);
